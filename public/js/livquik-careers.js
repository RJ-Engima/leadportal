$(document).ready(function () {
  ...
  $("#filtertblData").on("click", function () {
    ...
    $.ajax({
      url: "/livquik/careers/filterbyDate",
      data: {
      ...
      success: function (data) {
        ...
        data.map((i, index) => {
          let tableData = `
                    ...
                    <td>${i.phone}</td>
                    <td>${i.position}</td>
                    <td class="td-center">${'<button id="downlaod" class="dnbtn"><i class="fa fa-download"></i></button>'}</td>
                    </<tr>
                `;
  ...
  let array = [];
  $(document).on("click", "#downlaod", function (e) {
    e.preventDefault();
    ...
    let lastname = array[3];
    let phone = array[5];
    $.ajax({
      url: "/livquik/careers/getFiles",
      type: "POST",
      headers: { "CSRF-Token": enc_token, Authorization: `bearer ${token}` },
      data: {
        firstname: firstname,
      ...
      success: function (data) {
        ...
        a.href = "data:application/pdf;base64," + data.base64; //Image Base64 Goes here
        a.download = firstname+"_"+lastname+"-resume."+ data.ext; //File name Here
        a.click()
      },
      error: function (error) {
        $("#logMsg").removeClass("hide").addClass("dangerAlert").show();
